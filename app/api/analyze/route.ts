...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case 'telegram':
        systemPrompt = 'Ты профессиональный копирайтер для Telegram. Твоя задача - создавать КОРОТКИЕ, интересные посты для социальной сети. НЕ переводи текст дословно, а создавай новый пост на основе статьи. Пост должен быть компактным (не более 300-400 слов), с эмодзи, хештегами и ссылкой на источник.'
        const sourceLink = sourceUrl ? `\n\nОБЯЗАТЕЛЬНО: В самом конце поста, после всех хештегов, добавь ссылку на источник в формате:\n\n📎 Источник: ${sourceUrl}` : ''
        userPrompt = `Создай КОРОТКИЙ пост для Telegram на русском языке на основе следующей статьи. 

ВАЖНО:
- НЕ переводи текст дословно
- Создай НОВЫЙ пост, который кратко передает суть статьи
...
- Пост должен быть компактным (не более 300-400 слов)${sourceLink}
